				...
				<tbody className="bg-white divide-y divide-gray-200">
					{auditLogs.map((log) => (
						<tr key={log.id}>
							...
							<td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{log.parking_access?.guest_name || 'N/A'}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{log.gates?.name || 'N/A'}</td>
						</tr>
					))}
				</tbody>
			</table>
